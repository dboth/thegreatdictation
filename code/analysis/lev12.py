...
#What the matrix looks like after considering all additional rules: each field contains a list of informations. first information is 0 (no match), 1 (match). All other operations and the first line and column of the matrix already contain fields just as described below:
#After filling the matrix: Any field has to look as follows: (x,y,n, "operation"), where y and x are the starting field coordinates from which the operation leads to the field where we are now
#and n is the cost to get from the starting field to target field. While calculating the shortest path, all of these informations are considered and the minimum is chosen.
...
def levenshtein(string1, string2, debug):
    ...
    #get other long range information here as appended lists in the target field of the matrix in the form [y-value start field, x-value start field, cost to get from start field to target field]
   
    
    #get switched letters
    for i in range(1,len(string1)):
        for j in range(len(string2)-1):
            if string1[i] == string2[j]:
                if string1[i-1] == string2[j+1]:
                    matrix[i+1][j+2].append(matrix_field(i-1,j,1,"switch"))
    ...
    for x in range(1,len(string1)+1):				#in this loop, we calculate field x,y of the matrix
        for y in range(1,len(string2)+1):
            ...
            for i in range(len(poss)-1):										#now minim is exchanged whenever a shorter way is found
		minim = min(minim, matrix_field(poss[i][0], poss[i][1], matrix[poss[i][0]][poss[i][1]][2] +poss[i][2], poss[i][3]), key=itemgetter(2))
	    matrix[x][y] = minim
    
    #remove prints if debug is false
    #if debug == True:			
	#for line in matrix:
	    #print line
	#print "\n"   
